@tool @icon("uid://ei0lvq3hy8nn")
class_name HitboxComponent3D extends Area3D
## A unified 3D component that can act as either a hitbox or hurtbox.
...
signal hit_hurtbox(hurtbox: HitboxComponent3D, hit_effect: HitEffect)
## Emitted when this hurtbox is hit by a hitbox.
## hit_effect is the fully resolved resource from the attacker.
signal hit_by_hitbox(hitbox: HitboxComponent3D, hit_effect: HitEffect)
...
@export_flags_3d_physics var hit_layer: int = 0:
	set(v):
		hit_layer = v
		_setup_collision_layers()
...
func _on_area_entered(other_area: Area3D) -> void:
	if disable_collisions or other_area is not HitboxComponent3D:
		return

	var hurtbox := other_area as HitboxComponent3D
...
func _handle_hit(hurtbox: HitboxComponent3D) -> void:
...
func _notify_hit_by(hitbox: HitboxComponent3D, resolved_hit_effect: HitEffect) -> void:
...
## Override in subclasses to resolve specialized hit effects.
